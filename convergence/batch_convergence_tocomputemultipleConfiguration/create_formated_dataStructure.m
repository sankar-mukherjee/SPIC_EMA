...
% config = {'all_scores_comp-128_regularizationFactor-3_audio.txt'};
% 
% project.convergence_threshold = 1.5;
...
for con=1:length(config)
    ...
    label = {'subject','partner','session','word','convergence','divergence','nochange','effort_score','other_model_idx'};
    ...
    for i=1:10
        ...
        pre_model_score{b} = gmm_score.score(a,b);
    end
    
    feature = nan(length(gmm_score.seg),5);
    for i=1:length(data)
...
%         sub_to_partner = (pre_sub_score_partner_model - score_partner_model) / (pre_sub_score_partner_model - pre_partner_score_partner_model);
        ...
        feature(i,:) = [effort_score other_model_idx];
